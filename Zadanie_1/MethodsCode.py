...
def insert(fio, age, tel, otd):
    ...
    try:
        ...
        csv_file.append({'ном': int(mx['ном']) + 1, 'фио': fio, 'возраст': age, 'телефон': tel, 'отдел практики': otd})
...
def save():
    with open('data.csv', "w", encoding="utf-8", newline="") as file:
        columns = ['ном', 'фио', 'возраст', 'телефон', 'отдел практики']
...
def show_csv():
    if len(csv_file) == 0:
        ...
        print('{:<5}{:<25}{:<8}{:<12}{:<15}'.format(
            'ном', 'фио', 'возраст', 'телефон', 'отдел практики'
        ))
        for el in csv_file:
